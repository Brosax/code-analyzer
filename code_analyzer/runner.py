...
def analyze(
    ...
    control = RunControl(CancellationToken(), llm_jobs=int(config["llm"].get("jobs") or 1), decider=decider)
...
def _analyze(
    ...
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "build_context": manifest_block(config["build"]["assist"], "pending" if config["build"]["assist"] != "off" else "off", []),
        "artifacts": [],
    }
    ...
    # The executables the static lane resolved, for the build-context re-run.
    ...
    def run_static() -> None:
        ...
        for tool_index, name in enumerate(requested_names, 1):
            ...
            resolved_tools[name] = (resolved, version)
            with manifest_lock:
        ...
        if not interrupted and not cancellation.cancelled:
            run_build_context()
