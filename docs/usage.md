...
已运行时长、静态与 LLM 的完成数、token 预算），**总进度条**，其下**每条泳道一条
进度条**（静态分析按工具数、LLM 扫描按单元数），再下面是**速度条**（模型名、
tok/s、峰值、在途会话、已测量的输入/输出 token、请求数、ETA），然后是**扫描流程**
面板与右侧面板（对话 / 日志 / LLM 明细 / 问题）。

总进度是各泳道的加权和（权重见 `runner.WEIGHTS_*`）；它回答"这次运行还要多久"，
回答不了"LLM 走到哪了"。所以两者都画出来，谁也不必替谁说话。
...
| `F3` / `F4` | 切换右侧面板（对话 / 日志 / LLM 明细 / 问题）/ 切换日志过滤（全部 / 警告 / 错误） |
| `F6` | 在对话面板中显示 / 隐藏发送给模型的提示词（默认隐藏） |
| `Ctrl+C` | 协作式取消 |
...
### 3.2 对话面板：模型答了什么，答得多快

启用 LLM 泳道的运行**默认打开对话面板**（纯静态运行没有对话，仍然默认打开日志）。
它把一次运行摊成一场对话：每个扫描单元一"轮"，模型的回复逐 chunk 流入，工具调用
和重试就地标注，会话结束后在下方给出这一轮的账。

```text
╭─ 对话 · 已答 3/4 · 缓存 1 · F6 显示提示词 ───────────────────────────╮
│ ✓ llm-memory-safety · src/parse.c · 1/4 · critical · 完成            │
│   {"findings": [{"category": "unsafe-copy", "line_start": 14, …}]}   │
│   14.0 tok/s（测量） · 首字 2.1s · 输出 2000 tok · 输入 3969 tok      │
│   · 耗时 142.6s · 发现 1                                             │
│ ● llm-memory-safety · src/dup.c · 4/4 · high · 接收中 · 12s          │
│   ⚒ 工具调用 read                                                    │
│   {"findings": [                                                     │
│   3.1 tok/s（估算） · 首字 1.4s · 输出 ~28 tok                        │
╰──────────────────────────────────────────────────────────────────────╯
```

**两种速度，永不混淆。** 回复还在流入时只有字符数可数，面板按 `字符/4`（预算用的
同一除数）给出**估算**并标注"（估算）"；会话结束、provider 报回自己的
`outputTokens` 之后，改用 `输出 token / 会话耗时` 的**测量**值并标注"（测量）"。
把其中一个当成另一个显示，等于报告一个没人测过的吞吐量。速度条上的 tok/s 同理：
有回复在流时是最近 10 秒窗口的估算，安静下来则退回最近若干会话的均值，窗口过期后
不再显示——**停住的 provider 不该看起来很快**。

`F6` 打开提示词。默认关闭：一条提示词是整个扫描单元加上下文和 skill，正在看回复的
人不需要在每条回复前重读一遍问题。面板里显示的是**预览**——发送方按块截断（每块
最多 24 行、总计 4000 字符）并就地标注省略了多少行；完整提示词在报告目录的
`llm/units/<unit_id>.json` 里，会话的 `request.json` 记录其 `prompt_sha256`。
提示词预览**不进实时日志**（一条提示词会把一行一事件的日志整页埋掉），但和回复一样
会作为 `output` 事件进入 `events.jsonl`，`serve` 的实时页读的就是它。

对话面板与实时页读的是同一份事件流。`output` 事件的 `stream` 字段说明这一行是模型做的
哪一类事——两个前端因此不必各自去解析带前缀的文本行：

| `stream` | 内容 |
|---|---|
| `prompt` | 发送给模型的提示词预览；`data` 携带整条提示词的 `chars` / `lines` / `estimated_tokens` / `blocks` |
| `answer` | 回复本身，逐 chunk；同一轮内后一个 chunk 续接前一个 chunk 所在的行（一份 JSON 不是一行一块） |
| `tool` | agent 发起的工具调用名 |
| `note` | 关于这次请求的说明，目前是 provider 重试（`retry k/N: CODE message`） |
| `stdout` / `stderr` | 原生分析器的输出，与对话无关，只进日志面板 |

单元结束的 `unit` 事件在 `data.usage` 里带上 provider 自己的 `prompt_tokens` /
`completion_tokens` / `requests`，测量速度就是从这里来的。阶段级的 token 账在心跳里
发布，所以**最后一个会话的用量落在最后一次心跳之后**；两个前端都取"心跳账本"与
"各轮用量之和"中较大的那个——两者都不会超过真值，谁看得更全就用谁。

对话面板是**活的视图，不是证据**：最多保留 240 轮、每轮回复最多 400 行，被折叠的
用 `… 更早的 N 个单元` 汇总；正在流入的轮永远不会被折叠掉。完整的提示词、回复和
provider 计数都在报告目录里。终端过载时先丢回复 chunk，绝不丢状态事件——计数器和
流程图不会因为界面忙而说错话。

只有和模型对话的 producer 会出现在这里：原生分析器（cppcheck / flawfinder / splint）
发出的单元事件形状完全一样，但一个子进程不是一次对话，它们仍然只在日志和流程图里。
