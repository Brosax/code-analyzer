...
按代码审计，这是 `doctor` / `preflight` / `config` / `model` 四个。（决定 E5 当时写的是
「就这 3 个」，那时 `/model` 还不存在；它只向端点要一次模型列表，不发起生成，所以按同一条规则
落在同一侧。这个集合是从声明的效果**派生**的，不是一张手写名单。）其余全部确认一次，并在确认框里
...
`skill` 工具去取自己的说明，把六步预算全烧光）；`[llm] intent_reasoning` 打开之后，
模型会先想一会儿，实测到过 **52 秒**（输出 590 token，其中大部分是思维链）。所以等待
必须诚实：

```text
› 帮我看看哪些单元最值得先扫
  ↳ → 模型
  ⠹ 正在理解这句话… 24s · 模型在思考 · Ctrl+C 放弃
    qwen3.8:27b · ≈17.6 tok/s（估算） · 上次 21.7s（本会话测量）
    ▾ 思维链 · 4199 字符
    │ …
...
- **等待时的 tok/s 是估算，答完后的是测量，两者永不混淆。** 等待中的 `≈N tok/s（估算）`
  按 `字符 / 4`（预算用的同一除数）算，第一个 token 到达之后才出现；答案落地时换成
  provider 自己报的 `outputTokens ÷ 整个往返`，标成**（测量）**。同一条规则、同一套措辞在
  扫描界面的 `chat.Turn.speed()` 里已经用了很久——把估算当测量显示，等于凭空发明一个
  没人量过的吞吐。思维链**计入**这两个数：它按输出计费，不算进去会把一个会思考的模型
  报成慢好几倍。
...
`/model`（别名 `/模型`）**不发起生成**——它读配置，并向端点要一次模型列表，就是 preflight
一直在做的那次请求。所以它是免费的、秒回的，模型也可以自己去跑它。

```text
› /model
qwen3.8:27b @ http://192.168.5.10:11434/v1
  模型：qwen3.8:27b
  端点：http://192.168.5.10:11434/v1（profile: gpu-host）
  运行时：可用 · SDK 0.1.1rc1
  凭据：可用（none (keyless)）
  连接：可达，端点列出 10 个模型；配置的这个在其中
  上下文窗口：配置 32768，端点提供 24576
    端点只提供 24576 个 token，而 [llm] context_window 配的是 32768：超出的提示词会被静默截断
  本会话测得的速度：
    · 一次生成：24.6 tok/s，整个请求 8.2s（/llm-doctor 实测，312s 前）
    · 理解一句话的往返：52.1s，11.3 tok/s（实测）
    · 扫描时的模型对话：18.3 tok/s（会话均值，实测）
```

- **速度只来自实测，没测过就说没测过。** 上面三行分别来自 `/llm-doctor` 的基准、上一次
  自由文本的往返、以及上一次扫描的模型对话；一条都没有时它直说「本会话还没有测到任何速度」，
  **不拿配置或历史猜一个数**。想要一个真数字就跑 `/llm-doctor`——那是一次真实生成
  （计费、实测 18–52 秒），所以它要你自己敲。
- **`/llm-doctor` 的结果现在也进对话。** 它一直在测 tok/s 和延迟，但此前只有 CLI 渲染，
  对话里只剩一行 `模型 @ 端点`。两个前端现在共用 `llm.doctor.describe()` 这一个渲染函数。
- **底部状态栏常驻显示** `qwen3.8:27b ✓ 24.6 tok/s`：名字、闸门上次的判定、以及测到的速度。
  它读的是闸门缓存，**自己从不探测**——状态栏不该能花掉一次网络往返。没探测过就只显示
  名字，不显示对错：「还没问过」是第三种状态，画成「不可达」是在撒谎。
- 思考中的那一行也带上模型名与实时速率：
  `qwen3.8:27b · ≈17.6 tok/s（估算） · 上次 21.7s（本会话测量）`。
- **答案落地的地方说明这次往返的代价**，就在提议块的第一行：

  ```text
  模型理解为（只读，直接执行）：
    qwen3.8:27b · 52.1s · 11.3 tok/s（测量） · 输出 590 token
    [x] /doctor
  ```

  这个数一直被测着，却从来没显示过——只有**下一次**等待会以「上次 21.7s」的形式提一句。
...
5. **决定**：TUI 弹出逐项勾选的对话框；终端上是 `[y/N]`；无 TTY 的运行只记录不应用，
   ...
   **在对话框上敲斜杠命令仍然是命令，不是答案**——此前待答问题会吞掉一切输入，而对一个
   `select` 问题来说「既不是 y 也不是编号」等于**拒绝**，所以想问一句 `/help` 就把补丁扔了。
   只对答案是封闭集合的问题这样处理；`text` 问题问的是路径或参数，`/usr/include` 在那里
   是合法答案而不是命令。
...
### 10.1 `respect_gitignore` 支持哪些规则

`[source] respect_gitignore` 默认 `false`。打开后，扫描按 Git 的语义排除
被忽略的路径：

- **规则来源**：只读源码树内部的 `.gitignore`，每一层目录都读。不读
  `.git/info/exclude`，不读全局 `core.excludesFile`，也不看 index —— 它们
  在被扫描的树之外，读了会让同一份源码在两台机器上得到不同的扫描范围。
- **规则语义**：注释、空行、`!` 取反、`\#` 与 `\!` 转义、未转义的行尾空格、
  目录专用的结尾 `/`、开头或中间的 `/` 表示相对本层 `.gitignore` 锚定、
  `*`、`?`、`[abc]`、`[!abc]`、POSIX 字符类（`[[:digit:]]`、`[[:upper:]]`
  等十二种），以及跨目录的 `**`（`**/x`、`x/**`、`a/**/b`）。
- **优先级**：深层 `.gitignore` 覆盖浅层，同一文件内最后匹配的规则生效。
  与 Git 一致，父目录已被排除时，子文件的取反规则不能把它救回来。
- **谁更大**：`[source] exclude`、内建的默认排除目录（`.git`、`node_modules`、
  `build` 等）以及位于源码树内部的报告输出目录，优先级都高于 `.gitignore`
  的取反规则。
- **读不到规则时**：存在但读不出的 `.gitignore` 记为一条范围异常，本次扫描
  的范围判定为不完整（第 13 节），而不是当作"没有规则"继续。
...
"无法验证"是双向的：某个文件只在一次发现里读不到，另一次读到了，既不算被
删除也不算新增——文件一直都在，变的是我们能不能读它。这类路径记在
`changes.unverified` 里，源码稳定性记为未知。
