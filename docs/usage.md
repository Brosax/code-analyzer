...
**确定性解析器是主干**，离线可用、按键即响应，永远不找模型。四种写法：

| 你输入 | 解析为 |
|---|---|
| `/scan ~/fw --llm-jobs 4` | 斜杠命令。**尾巴交给 `code-analyzer analyze` 自己的 parser**，所以它接受的 flag 与子命令逐字相同，`--llm-jobs 0` 也用同一句话拒绝 |
| `~/fw/tfm` | 裸路径 → 提议扫描它（**只是提议**，仍要确认） |
| `扫描 ~/fw` | 简写：一个动词 + 一个能解析的路径 |
| `/set llm.jobs 4` | 改一项配置 |

解析不了就**说解析不了，绝不猜**：

- 一句话同时像两个动词 → 两个都列出来问你指哪个；
- 一个含 `manifest.json` 的目录 → 列出对一次已完成运行能做的全部五件事
  （`llm-resume` / `assess` / `tools-resume` / `recover-report` / `serve`），
  因为在它们之间替你选是抛硬币，而你一秒就能定；
- 其余 → 报告「没看懂」，并指向 `/help` 与 `/ask`。

注意 `/` 开头的既可能是命令也可能是**绝对路径**。命令是一个由字母数字和连字符组成
的词；`/home/ubuntu/fw` 是路径——这是操作员最可能输入的第一样东西。

### 3.2 `/ask`：把一句话交给模型

唯一需要 provider 的入口。模型可以提议注册表里的**任何** action，但：

- 目录（catalogue）是从注册表**生成**的，所以模型说得出的 action 一定存在；
- 目录之外的 action 按名字丢弃；
- 提议的配置改动必须是真实叶子、可写、且过 `validate_config`，否则丢弃；
- 目标目录不存在也丢弃；
- **每一条丢弃都写明理由**——悄悄少掉一半步骤比说明拒绝了什么更糟；
- 每一步**默认不勾选**，勾选后变成「你本来也能敲出来的那条斜杠命令」再执行，
  该确认的仍然各自确认一次。

模型拿到的是：目录、当前路径、与默认值不同的配置项、以及被围栏标成 DATA 的那句
话。**拿不到 finding、拿不到分析器输出、拿不到源码**——`docs/platform-architecture.md`
§2.3 那条硬规则不因这条通道放宽，skill 的 frontmatter 也不授予 `fs`。

provider 不可达是**闸门而不是异常**：`/ask` 说明原因，其余命令照常。非 TTY 下
`/ask` 直接拒绝，免得一次 provider 故障改变无人值守的退出码。

实测（2026-09-03，qwen3.8:27b）：一次 `/ask` 约 58–79 秒——这正是主干必须确定性的
理由，「把并发改成 4」不该等半分钟。
...
`code-analyzer <你说的一句话>` 也走同一个解析器。但**裸路径在非 TTY 下只打印它将要
运行的那条命令然后退出 2**——一次扫描不是副作用。
