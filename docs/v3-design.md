...
| M7 | 2df5f01 | Juliet T1 48/48 已核实、接地失败 0%、10 分 44 秒 GPU；T2 点名 bad 函数 32 任务、14 条晋升，CWE121/401/457/476 全部成为已接地条目；TF-M 两个 SFR（SIP、SS）60 分钟：计划 1693 = 已审 226 + 未排上 1467（全部“GPU 额度用完”），GPU 3603.6 s，接地 224/226；T1 优先，所以 60 分钟内没有轮到 T2；评估员抽审待做 |
| M8 | c6f2169 | 版本对比：TF-M v2.3.0 → HEAD（+203 次提交，同一构建上下文沿用）13,572 → 13,120 条：保留 12,710（同一发现 12,053、同锚点 657）、新增 410、消失 862（其中 573 条在 HEAD 已删除的 cryptocell-312-runtime），0.86 s；b.ai 真机待用户提供新 key |
...
- **打包**：`kernel/method.md`（agent 的系统提示）原先不在 package-data 里，非可编辑安装会缺（已在 main 修复）；`settings.toml [analyzers]` 原先读了不用（修复在 m9-cleanup 分支）。两处都有测试。
- **对话抢占审查**（Juliet，审查 job 与对话同一进程）：job 进行中连问三句，首字 32.0 / 4.8 / 9.9 s、答完 36.5 / 6.9 / 14.5 s（第一句含冷前缀）；job 记录 1 次抢占，44/44 完成。M0 探针在无抢占时测得 2/4 个后台请求把首字推到 53/104 s。
- **同一目录的多个记录器**：对话和审查各自给 `model/` 编号，第一次联测时撞号（`File exists: model/0009`）并让整个审查 job 失败。改为原子地占用目录、撞号就跳过；engine 也不再因为一个任务的意外异常丢掉整个 job。
- **job 编号跨重启连续**：服务器重启后编号从 J1 重来，账本里两个不同的 job 同名。现在每个 job 写 `job_started`，编号从账本里见过的最大值往后排。
